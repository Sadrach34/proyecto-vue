<template>
    <h3>Propiedades Computadas</h3>
    <input type="text" v-model="valor" />
    {{ valorCalculado }}
    <ResultadoSection v-if="mostratElemento" :resultado="valorCalculado" />
    <div v-else-if="isNaN(valor)">Debe de ser un numero para calcular sonso</div>
    <div v-else>Aqui no se muestra nada</div>
    
</template>

<script setup>
    import { ref, computed } from 'vue';
    import ResultadoSection from '../components/ResultadoSection.vue';

    const valor = ref('');
    const valorCalculado = computed(() => {
        return valor.value * 2
    });


const mostratElemento = computed(() => {
    return valor.value > 10;
});
</script>

<style scoped>
</style>